...
2. Updates its status to the status selected by n8n.
...
The n8n expression selects `MAINTENANCE` for `service` and `AVAILABLE` for `main`. Keep the API URL reachable from the n8n host; `localhost` only works when n8n and the API run on the same machine.
